...
| `kv-bytes-per-token/` | ml-explore/mlx-lm, vllm-project/vllm-metal | KV cache bytes per token by architecture family, verified against mlx_lm's own cache objects on 21 families. |
